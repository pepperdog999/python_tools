...
#: Python 2.x?
is_py2 = (int(_python_ver.split('.')[0]) == 2)
#: Python 3.x?
is_py3 = (int(_python_ver.split('.')[0]) == 3)
...
if __name__ == "__main__":
    ...
    path = 'img/'
    if len(argv)>2 and argv[1].startswith('--'): # 指定--path
        option = argv[1][2:]
    ...
    if len(uf) > 0: #用于接收外部参数
        upload(uf, path)
    else: #用于测试
        file = './uploader_qiniu_sdk.py'
        uf.append(file)
        upload(uf, path)